...
class MailerTest < ActionMailer::TestCase
  ...
  context 'order_transferred' do
    setup { @message = Mailer.create_order_transferred(@order = Factory(:order)) }
    should('be to email')               { assert_equal [@order.email], @message.to }
    should('be from application email') { assert_equal [Mailer::FROM_ADDRESS], @message.from_addrs.map(&:to_s) }
  end
  ...
  context 'overdue_batches' do
    ...
    should('be to contact email') do
      assert_equal [Mailer::CONTACT_ADDRESS], @message.to_addrs.map(&:to_s)
    end
